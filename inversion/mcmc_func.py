# -*- coding: utf-8 -*-
"""
@author: Eric Laloy <elaloy@sckcen.be>, October 2016.
"""
import numpy as np
...
def RunFoward(X,MCMCPar,Measurement,ModelName,Extra,DNN=None):
    ...
    if ModelName=='theoretical_case_mvn':
        ...
        noise_input = X.astype('float32')
        del X
        X=DNN.gen_from_enc(noise_input,DNN.nloop,DNN.gen_from_noise,DNN.noise_adjust,
                           DNN.ae_encode_mu,DNN.ae_encode_log_sigma,DNN.threshold)
...
def forward_model_flow(X,n,par):
    ...
    X=X[0,1:]
    main_dir=os.getcwd()
    model_dir=main_dir + "/modflow_"+str(idf)
    os.chdir(model_dir)
    if par[0][0]==1:
        X[X==0]=1e-4;X[X==1]=1e-2
        #X=1e-4*np.exp(np.log(100.0)*X)
    elif par[0][0]==3:
        X[X==0]=1e-6;X[X==1]=1e-4
        #X=1e-6*np.exp(np.log(100.0)*X)
    elif par[0][0]==2:
        pass # This is for a 2D transient simulation that was not shown in the paper because of space constraints
    if  par[0][0]==1 or par[0][0]==2:
        filename = r'fmod.mlt'
        template='# MF2005 multiplication file \n1 \nmult \nINTERNAL 1.0 (free) 0'
        f = open(filename, 'w') 
        np.savetxt(filename, np.reshape(X,(100,100)),delimiter=' ', fmt='%1.4e',header=template,comments='')
        f.close()
    if par[0][0]==3:
        num_layer=27
        X3d=np.reshape(X,(27,32,30))
        ...
        for i in range(0,num_layer): 
            ...
            f.write('\n')
            np.savetxt(f, np.reshape(X3d[i,:,:],(32,30)), fmt='%1.4e')
        f.close()
    if par[0][0]==1:
        external_code='mf2005.exe fmod.nam' #Windows
        #external_code='./mf2005 fmod.nam' #Linux
    elif par[0][0]==2:
        external_code='MODFLOW-NWT_64.exe fmod_nwt_tr.nam' #Windows
        #external_code='./MODFLOW-NWT_64 fmod_nwt_tr.nam'  #Linux
    elif par[0][0]==3:
    ...
    if par[0][0]==1:
        ...
        fx=np.loadtxt('fmod.sim',skiprows=1,usecols=(0,)).reshape((1,n))
    elif par[0][0]==3:
        idx=par[1][0]-1
