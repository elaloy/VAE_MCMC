...
class Sampler:
    ...
    def __init__(self, CaseStudy=0,seq = 3,ndraw=10000,thin = 1,  nCR = 3, 
        ...
        if self.CaseStudy==3 or self.CaseStudy==2:  
            ...
            DNN=AttrDict()
            DNN.ndr=self.ndim
            DNN.ae_encode_mu, DNN.ae_encode_log_sigma, DNN.noise_adjust, DNN.gen_from_noise, DNN.gen_from_enc = buildnet(weight_file,DNN.ndr)
            DNN.threshold=True
            DNN.nloop=10
            self.DNN=DNN
        ...
        if self.CaseStudy==3:   
            ...
            Extra.trueK=np.swapaxes(np.reshape(np.loadtxt('trueK_3D.txt'),(30,32,27),order="F"),0,2)
